...
class SmartMealPlannerCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final headerGrad =
        isDark
            ? LinearGradient(
              colors: [p.withValues(alpha: 0.12), const Color(0xFF121218)],
              begin: Alignment.topLeft,
              end: Alignment.bottomRight,
            )
            : LinearGradient(
              colors: [p.withValues(alpha: 0.04), Colors.white],
              begin: Alignment.topLeft,
              end: Alignment.bottomRight,
            );
    final headerBorder =
        isDark ? p.withValues(alpha: 0.15) : const Color(0xFFE2E8F0);
    ...
    final refreshBg =
        isDark ? const Color(0xFF1A1A24) : const Color(0xFFF8FAFC);
      ...
      child: Column(
        ...
        children: [
                ...
                Container(
                  ...
                  child: FractionallySizedBox(
                    ...
                    child: Container(
                      decoration: BoxDecoration(
                        gradient: LinearGradient(
                          colors: [p, p.withValues(alpha: 0.6)],
                        ),
                        borderRadius: BorderRadius.circular(99),
                      ),
                    ),
          ...
          Padding(
            ...
            child:
                isTeaser
                    ? _buildTeaser(context, p)
                    : Column(
                      children: List.generate(
                        meals.length,
                        (i) => Padding(
                          padding: EdgeInsets.only(
                            bottom: i == meals.length - 1 ? 0 : 8,
                          ),
                          child: _buildMealSlot(context, meals[i], p),
                        ),
                      ),
                    ),
          ),
  ...
  Widget _buildTeaser(BuildContext context, Color p) {
    ...
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final hidden = meals.length > 2 ? meals.sublist(2) : const <MealSlot>[];
    final hiddenKcal = hidden.fold<int>(0, (sum, slot) => sum + slot.kcal);
      ...
      children: [
        if (meals.isNotEmpty)
          Padding(
            ...
            child: _buildMealSlot(context, meals[0], p),
          ),
        if (meals.length > 1)
          Padding(
            padding: const EdgeInsets.only(bottom: 8),
            child: _buildMealSlot(context, meals[1], p),
          ),
        // The rest of the day is a genuine Pro feature, so it stays locked —
        // but it is stated rather than rendered as blurred mush. Naming what
        // is behind the lock is what creates appetite; grey shapes don't.
        if (hidden.isNotEmpty)
          Container(
            width: double.infinity,
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
            decoration: BoxDecoration(
              color: p.withValues(alpha: isDark ? 0.08 : 0.05),
              borderRadius: BorderRadius.circular(12),
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    l10n.planner_teaser_locked_summary(
                      hidden.length,
                      _formatKcal(hiddenKcal),
                    ),
                    style: TextStyle(
                      fontSize: 12,
                      fontWeight: FontWeight.w700,
  ...
  static String _formatKcal(int v) {
    if (v < 1000) return '$v';
  ...
  Widget _buildMealSlot(BuildContext context, MealSlot slot, Color p) {
    ...
    if (slot.status == MealSlotStatus.done) {
      ...
      border = BorderSide(
        color: isDark ? const Color(0xFF1A1A24) : const Color(0xFFE2E8F0),
      );
      ...
      border = BorderSide(
        color: p.withValues(alpha: isDark ? 0.4 : 0.5),
        width: 1.4,
      );
      iconDeco = BoxDecoration(
      ...
      border = BorderSide(
        color: isDark ? const Color(0xFF1A1A22) : const Color(0xFFE2E8F0),
      );
      iconDeco = BoxDecoration(
      ...
      child: Row(
        children: [
          Container(
            ...
            height: 36,
            decoration: iconDeco,
            child: Center(child: Icon(icon, color: iconC, size: 18)),
          ),
    ...
    return AppScaleTap(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(12),
          border: Border.all(
            color: widget.primary.withValues(alpha: isDark ? 0.3 : 0.5),
            width: 1,
          ),
        ),
        child: Row(
...
class MacroInsightCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cardBg = isDark ? const Color(0xFF121218) : const Color(0xFFFFFFFF);
    final borderC =
        isDark ? p.withValues(alpha: 0.25) : const Color(0xFFE2E8F0);
