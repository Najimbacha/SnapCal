...
class _HydrationSheetState extends ConsumerState<_HydrationSheet>
  ...
  Future<void> _addWater() async {
    ...
    if (newState != null && newState.todayTotal >= newState.goal) {
  ...
  Future<void> _undoAdd() async {
    ...
    if (!mounted) return;
    final newState = ref.read(waterProvider).valueOrNull;
    if (newState != null && newState.todayTotal <= 0) {
      _lastDrinkTime = null;
      _timeAgo = '';
    }
    _riseCtrl.forward(from: 0);
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer(
      builder: (ctx, ref, _) {
        ...
        final display =
            _riseCtrl.isAnimating
                ? _fromMl + (actual - _fromMl) * _riseAnim.value
                : actual.toDouble();
              ...
              Positioned(
                ...
                child: ClipRRect(
                  ...
                  child: WaterAnimation(
                    ...
                    frequency: 1.2,
                    speed: 2.5,
                    waterColor: AppColors.primary.withValues(
                      alpha: d ? 0.3 : 0.15,
                    ),
                    gradientColors:
                        d
                            ...
                            : [
                              ...
                              AppColors.emeraldLight.withValues(alpha: 0.06),
                            ],
                    enableRipple: true,
                    enableShader: true,
                    enableSecondWave: true,
                    secondWaveColor: AppColors.primary.withValues(
                      alpha: d ? 0.15 : 0.08,
                    ),
                                  ...
                                  return Expanded(
                                    child: Padding(
                                      padding: EdgeInsets.only(
                                        left: ml == _presets.first ? 0 : 8,
                                        right: ml == _presets.last ? 0 : 8,
                                      ),
