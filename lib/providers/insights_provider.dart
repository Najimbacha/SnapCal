  ...
  Future<void> generateWeeklyReport({required String languageCode}) async {
    ...
    state = await AsyncValue.guard(() async {
      ...
      for (int i = 6; i >= 0; i--) {
        ...
        final dayMeals =
            weeklyMeals.where((m) => m.dateString == dateStr).toList();
        ...
        dailyFat.add(dayFat);
        final dayWaterLogs =
            weeklyWaterLogs.where((l) => l.dateString == dateStr).toList();
      ...
      final avgCal = avg(dailyCalories),
          avgPro = avg(dailyProtein),
          avgCarb = avg(dailyCarbs);
